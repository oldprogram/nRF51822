...
void My_PWM_Init()
{
	...
	pwm_config.mode             = PWM_MODE_LED_255;	 // 8-bit resolution, 122Hz PWM frequency, 32kHz timer frequency (prescaler 9)
	pwm_config.num_channels     = 1;						 //最多3个
	pwm_config.gpio_num[0]      = LCD_BCK_LED_EN;                    //该设置为使用哪个IO口作为PWM输出

  
	nrf_pwm_init(&pwm_config); //pwm初始化
