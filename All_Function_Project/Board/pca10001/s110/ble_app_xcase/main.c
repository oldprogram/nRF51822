...
/** @file
 *
 * @defgroup ble_sdk_app_hrs_eval_main.c
 * @{
...
#include "flash.h"
//#include "sensor.h"
#define    SENSOR_Disable_POWER    nrf_gpio_pin_clear(SEN_POWER_EN)   //片选有效
#define    SENSOR_Enable_POWER   nrf_gpio_pin_set(SEN_POWER_EN)     //片选无效
...
#define DEVICE_NAME                          "Xcase_Smart1"                              /**< Name of device. Will be included in the advertising data. */
#define MANUFACTURER_NAME                    "Witlink"                     /**< Manufacturer. Will be passed to Device Information Service. */
...
#define BUTTON_DETECTION_DELAY               APP_TIMER_TICKS(100, APP_TIMER_PRESCALER)  /**< Delay from a GPIOTE event until a button is reported as pushed (in number of timer ticks). */
#define SENSOR_DETECTION_DELAY               APP_TIMER_TICKS(1, APP_TIMER_PRESCALER)
...
static volatile uint8_t                      sensor_healthy_state;
...
void bt_write_loopback(uint16_t data)//at most 2 bytes
{
    uint32_t err_code;

    err_code = ble_hrs_heart_rate_measurement_send(&m_hrs, data);
...
static void services_init(void)
{
    ...
    body_sensor_location = sensor_healthy_state;
...
static void buttons_init(void)
{
    ...
    // the buttons.
    static app_button_cfg_t sensors[] =
    {
