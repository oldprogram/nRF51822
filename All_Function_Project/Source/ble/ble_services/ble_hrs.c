...
#if defined(BT_WRITE_TEST)
extern void bt_write_loopback(uint16_t data);//at most 2 bytes
#endif
...
static void xcase_cmd_handler(ble_xcase_cmd_type_t cmd_type)
{
	switch (cmd_type)
				{
					case BLE_XCASE_CMD_OFF:
						if(nrf_gpio_pin_read(SEN_POWER_EN))
						    nrf_gpio_pin_clear(SEN_POWER_EN);//disable sensor
						break;
					case BLE_XCASE_CMD_ON:
						if(nrf_gpio_pin_read(SEN_POWER_EN) == 0 )
...
static void data_rcv_handler(ble_hrs_t * p_lbs,uint16_t len, uint8_t *rcv_data)
{
	#if defined(BT_WRITE_TEST)
	  uint16_t feedback_data;
	  feedback_data = (rcv_data[1]<<8)|rcv_data[0];
	#endif
	ble_xcase_rcv_type_t rcvd_type;
	ble_xcase_cmd_type_t cmd_type;
    if (len)
    {
        #if defined(BT_WRITE_TEST)
			      bt_write_loopback(len);//rcved data length
            bt_write_loopback(feedback_data);//first 2 bytes of rcvd data
        #endif
			  rcvd_type = (ble_xcase_rcv_type_t)rcv_data[0];
			  switch (rcvd_type)
				{
					case BLE_XCASE_EVT_DATA:
						//disp rcvd char 
						break;
					case BLE_XCASE_EVT_CMD:
						cmd_type = (ble_xcase_cmd_type_t)rcv_data[1];
					  xcase_cmd_handler(cmd_type);
						break;
					default:
						//error report
						break;
				}
			//call flash index function
    }
    else
    {
        ;
...
static void on_write(ble_hrs_t * p_hrs, ble_evt_t * p_ble_evt)
{
    ...
    if (p_evt_write->handle == p_hrs->hrm_handles.cccd_handle)
    {
		...
		if (p_evt_write->handle == p_hrs->data_rcv_handles.value_handle)
    {
        data_rcv_handler(p_hrs, p_evt_write->len, p_evt_write->data);
    }
}
 ...
 * @return      NRF_SUCCESS on success, otherwise an error code.
 */
static uint32_t hrm_data_rcv_char_add(ble_hrs_t * p_hrs, const ble_hrs_init_t * p_hrs_init)
{
    ble_gatts_char_md_t char_md;
    ble_gatts_attr_t    attr_char_value;
    ble_uuid_t          ble_uuid;
    ...
    attr_md.read_perm  = p_hrs_init->hrs_rcv_attr_md.read_perm;
    attr_md.write_perm = p_hrs_init->hrs_rcv_attr_md.write_perm;
    attr_md.vloc       = BLE_GATTS_VLOC_STACK;
    attr_md.rd_auth    = 0;
    attr_md.wr_auth    = 0;
    attr_md.vlen       = 1;
    ...
    attr_char_value.init_offs    = 0;
    attr_char_value.max_len      = MAX_DFU_PKT_LEN;//sizeof(uint8_t);//TBD
    attr_char_value.p_value      = NULL;
    
    return sd_ble_gatts_characteristic_add(p_hrs->service_handle,
                                           &char_md,
                                           &attr_char_value,
                                           &p_hrs->data_rcv_handles);
}
...
uint32_t ble_hrs_init(ble_hrs_t * p_hrs, const ble_hrs_init_t * p_hrs_init)
{
    ...
    if (p_hrs_init->p_body_sensor_location != NULL)
    {
		...
		err_code = hrm_data_rcv_char_add(p_hrs, p_hrs_init);
		if (err_code != NRF_SUCCESS)
    {
        return err_code;
    }
