...
/**@brief received data event type. */
typedef enum
{
    BLE_XCASE_EVT_DATA = 0xA5,                   /**< Heart Rate value notification enabled event. */
    BLE_XCASE_EVT_CMD = 0xA6                   /**< Heart Rate value notification disabled event. */
} ble_xcase_rcv_type_t;

/**@brief received data event type. */
typedef enum
{
    BLE_XCASE_CMD_OFF = 0,                   /**< Heart Rate value notification enabled event. */
    BLE_XCASE_CMD_ON = 1                   /**< Heart Rate value notification disabled event. */
} ble_xcase_cmd_type_t;
...
typedef void (*ble_hrm_rcv_handler_t) (ble_hrs_t * p_hrs, uint16_t len, uint8_t *rcv_data);
...
typedef struct
{
    ...
    ble_srv_security_mode_t      hrs_bsl_attr_md;                                      /**< Initial security level for body sensor location attribute */
	  ble_srv_security_mode_t      hrs_rcv_attr_md;                                      /**< Initial security level for heart rate service measurement attribute */
} ble_hrs_init_t;
...
typedef struct ble_hrs_s
{
    ...
    //ble_hrm_rcv_handler_t        data_rcv_handler; 
	  ble_gatts_char_handles_t     data_rcv_handles;  
