...
uint8_t SpiFlash_Write_Page(uint8_t *pBuffer, uint32_t WriteAddr, uint32_t WriteBytesNum)
{
    ...
    for(i = 0;i < WriteBytesNum; i++)
    {
        ...
        SpiFlash_WriteOneByte(pBuffer[i]);  
    }
    nrf_gpio_pin_set(28);
    ret = SpiFlash_Wait_Busy();                   
    if(ret != RET_SUCCESS)
...
uint8_t SpiFlash_Write_MorePage(uint8_t *pBuffer, uint32_t WriteAddr, uint32_t WriteBytesNum)
{
    ...
    PageByteRemain = 256 - WriteAddr%256;
...
void Spi_Flash_test(void)
{
	...
	SpiFlash_Write_Data("NRF",6,7,3);
	nrf_delay_ms(100);
	
	SpiFlash_Read_Data(FLASH_Buf,6 ,7 ,3);
